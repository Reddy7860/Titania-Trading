...
angel_script = pd.read_csv('~/Downloads/angel_script.csv')

client_data = pd.read_csv("~/Downloads/Reddy_Stocks_Application/Client_Details.csv")
...
for i in range(0,len(client_data)):
    ...
    if (os.path.isfile("/Users/apple/Downloads/Orders_Data/"+str(client_data.loc[i,"client_id"])+"/Updated_Targets/" + datetime.now(timezone("Asia/Kolkata")).strftime("%Y-%m-%d")+".csv")):
        previous_orders = pd.read_csv("/Users/apple/Downloads/Orders_Data/"+str(client_data.loc[i,"client_id"])+"/Updated_Targets/" + datetime.now(timezone("Asia/Kolkata")).strftime("%Y-%m-%d")+".csv")
    else:
        previous_orders = pd.DataFrame(columns = ['symboltoken', 'symbolname', 'instrumenttype','priceden','pricenum','genden','gennum','precision','multiplier','boardlotsize','exchange','producttype','tradingsymbol','symbolgroup','strikeprice','optiontype','expirydate','lotsize','cfbuyqty','cfsellqty','cfbuyamount','cfsellamount','buyavgprice','sellavgprice','avgnetprice','netvalue','netqty','totalbuyvalue','totalsellvalue','cfbuyavgprice','cfsellavgprice','totalbuyavgprice','totalsellavgprice','netprice','buyqty','sellqty','buyamount','sellamount','pnl','realised','unrealised','ltp','close','target_order_id','stop_loss_order_id','final_order_id','cancel_order_id','max_profit_percent','current_stoploss_percent','current_prob'])
    ...
    if(len(current_position_data) >0):
      ...
      left join previous_orders t2 on t1.tradingsymbol = t2.tradingsymbol
        ...
        current_position_data['prev_netqty'] = current_position_data['prev_netqty'].fillna(0)

        print(current_position_data)

        print(order_data)

        completed_orders = order_data[order_data['status'] == "complete"].sort_values(by ="exchorderupdatetime")

        completed_orders['row_number_bygroup'] = completed_orders.groupby(['tradingsymbol','transactiontype']).cumcount()+1
        ...
        if len(current_position_data) > 0 :
            for ord in range(0,len(current_position_data)):
                print("The Current Order is :",str(current_position_data.loc[ord,"tradingsymbol"]))

                if current_position_data.loc[ord,"tradingsymbol"] == "WIPRO28JUL22410CE" or current_position_data.loc[ord,"tradingsymbol"] == "NIFTY11AUG2217200PE"  or current_position_data.loc[ord,"tradingsymbol"] == "NIFTY22SEP2217800PE"  or current_position_data.loc[ord,"tradingsymbol"] == "NIFTY22SEP2217700PE":
                    print("Wipro order")
                else:
                    if (int(float(current_position_data.loc[ord,"netprice"])) > 0):
                        current_strike_completed = completed_orders[completed_orders["tradingsymbol"] == current_position_data.loc[ord,"tradingsymbol"]].sort_values(by ="exchorderupdatetime")

                        avg_order_price = 0

                        print("Open Positions")

                        open_position = pd.DataFrame(current_position_data.loc[ord,])
                        open_position = open_position.T
                        open_position.reset_index(inplace = True, drop = True)

                        print(open_position.loc[0,"tradingsymbol"])

                        executed_orders = order_data[(order_data['tradingsymbol'] == open_position.loc[0,"tradingsymbol"]) & (order_data['status'] == 'complete')]
                        ...
                        if executed_orders.empty:
                            ...
                            if open_position.loc[0,'tradingsymbol'][len(open_position.loc[0,'tradingsymbol']) - 2:] == "CE":
                                Signal = "BUY"
                            else:
                                Signal = "SELL"
                            ...
                            if executed_orders.loc[0,'tradingsymbol'][len(executed_orders.loc[0,'tradingsymbol']) - 2:] == "CE":
                                Signal = "BUY"
                            else:
                                Signal = "SELL"
                            ...
                            hms=dt.timedelta(hours=cr_date.hour,minutes=cr_date.minute,seconds=cr_date.second)

                            resolution=dt.timedelta(minutes=5)

                            time_sub = dt.timedelta(seconds=hms.seconds%resolution.seconds)

                            signal_time = cr_date - time_sub

                            signal_indicator = technical_data[technical_data['Datetime'] == signal_time.strftime('%Y-%m-%d %H:%M:%S')]
                            current_indicator = technical_data[technical_data['Datetime'] == max(technical_data['Datetime'])]

                            signal_indicator.reset_index(level=0, inplace=True,drop = True)
                            current_indicator.reset_index(level=0, inplace=True,drop = True)
                            ...
                            if Signal == "BUY":
                                initial_prob = signal_indicator.loc[0,'buy_probability']
                                current_prob = current_indicator.loc[0,'buy_probability']
                            else:
                                initial_prob = signal_indicator.loc[0,'sell_probability']
                                current_prob = current_indicator.loc[0,'sell_probability']
                        ...
                        if len(current_strike_completed) > 0:
                            n_occur = pd.DataFrame(current_strike_completed['row_number_bygroup'].value_counts())
                            n_occur['index'] = n_occur.index
                            open_orders = current_strike_completed[current_strike_completed['row_number_bygroup'].isin(n_occur[n_occur['row_number_bygroup'] == 1]['index'])]
                        ...
                        price_diff = (float(avg_order_price) - float(current_close))*100 / float(avg_order_price)

                        print("price_diff : ",str(price_diff))

                        print("Running the analysis for ",client_data.loc[i,"client_id"])
                        ...
                        if current_position_data.loc[ord, "max_profit_percent"] > 0:
                            profit_pct = max(current_position_data.loc[ord, "max_profit_percent"],
                                            0 if price_diff >= -3 else 
                                            3 if (price_diff >-3 and price_diff <= -7) else
                                            7 if (price_diff > -7 and price_diff <= -10) else
                                            10 if (price_diff >= -20 and price_diff <= -10) else 20)
                        else:
                        ...
                        current_position_data.loc[ord,"max_profit_percent"] = profit_pct
                        ...
                        elif target_order_id is not None and target_order_id == final_order_id:
                            orders_check = 'placed_target_order'
                            print("Target Order Placed")
                        elif stop_loss_order_id is not None  and stop_loss_order_id == final_order_id:
                        ...
                        if round(current_prob,2) <= 10:
                            ...
                            target_price = round(int(float(avg_order_price)) + 0.05 * int(float(avg_order_price)),0)

                            stop_loss_price = round(int(float(avg_order_price)) - 0.1 * int(float(avg_order_price)),0)

                            # price_diff = -1.5

                            current_position_data.loc[ord,'current_prob'] = current_prob

                            place_calculated_order(price_diff)


                            if price_diff > 20 :
                                print("The price difference is already greater than 20 %")
                                ...
                                if orders_check == 'place_initial_order':
                                    ...
                                    order_place_sl = place_stoploss_market(str(open_position.loc[0,"tradingsymbol"]),str(open_position.loc[0,"symboltoken"]),stop_loss_price,net_qty,avg_order_price)
                                    # order_place_sl = 1
                                    print("Placed the stoploss order : ",order_place_sl)
                                    current_position_data.loc[ord,"stop_loss_order_id"] = order_place_sl
                                    current_position_data.loc[ord,"final_order_id"] = order_place_sl
                                elif orders_check == 'placed_target_order':
                                    ## Cancel the target order as it went to loss

                                    stop_loss_price = current_close

                                    cancel_id = obj.cancelOrder(order_id = str(int(float(final_order_id))),
                                                       variety = "NORMAL"
                                                       )
                                    ...
                                    if int(middle_prob) > int(current_prob) :
                                        ...
                                        cancel_id = cancel_id['data']['orderid']
                                        print("Cancelled the Stoploss with higher Probability order : ",cancel_id)
                                ...
                                if orders_check == 'place_initial_order':
                                    ...
                                    order_place = place_target_order(str(open_position.loc[0,"tradingsymbol"]),str(open_position.loc[0,"symboltoken"]),target_price,net_qty)
                                    
                                    print("Placed the target order : ",order_place)
                                    
                                    current_position_data.loc[ord,"target_order_id"] = order_place
                                    current_position_data.loc[ord,"final_order_id"] = order_place
                        ...
                        elif round(current_prob,2) > 10 and round(current_prob,2) <= 40:
                            print("Current Prob : ",str(current_prob)," Middle Prob : ",str(middle_prob))
                            print("Target is 10% and SL 5%")

                            current_position_data.loc[ord,"current_stoploss_percent"] = 0.05
                            ...
                            target_price = round(int(float(avg_order_price)) + 0.10 * int(float(avg_order_price)),0)

                            stop_loss_price = round(int(float(avg_order_price)) - 0.05 * int(float(avg_order_price)),0)
                            ...
                            if price_diff > 25 :
                                print("The price difference is already greater than 25 %")

                                if orders_check == 'place_initial_order':
                                    stop_loss_price = current_close
                                    print("Current avg_order_price :",avg_order_price)
                                    # order_place_sl = place_stoploss_market(str(open_position.loc[0,"tradingsymbol"]),str(open_position.loc[0,"symboltoken"]),current_close,net_qty,avg_order_price)
                                    order_place_sl = place_stoploss_market(str(open_position.loc[0,"tradingsymbol"]),str(open_position.loc[0,"symboltoken"]),stop_loss_price,net_qty,int(float(avg_order_price)))
                                    # order_place_sl = 1
                                    print("Placed the stoploss order : ",order_place_sl)
                                    current_position_data.loc[ord,"stop_loss_order_id"] = order_place_sl
                                    current_position_data.loc[ord,"final_order_id"] = order_place_sl
                        ...
                        elif round(current_prob,2) > 40 and round(current_prob,2) < 80:

                            print("Current Probability : ",str(current_prob), " Middle Probability : ",str(middle_prob))

                            print("Target is 15% and SL 0%")

                            current_position_data.loc[ord,"current_stoploss_percent"] = 0.00
                            ...
                            target_price = round(int(float(avg_order_price)) + 0.15 * int(float(avg_order_price)),0)
                            ...
                            target_price = round(int(float(avg_order_price)) + 0.20 * int(float(avg_order_price)),0)
                            ...
                            current_position_data.loc[ord,"current_stoploss_percent"] = -0.10
                            ...
                            target_price = round(int(float(avg_order_price)) + 0.25 * int(float(avg_order_price)),0)
            ...
            current_position_data.to_csv("~/Downloads/Orders_Data/"+str(user_id)+"/Updated_Targets/"+str(datetime.now(timezone("Asia/Kolkata")).strftime("%Y-%m-%d"))+".csv")
