...
obj=SmartConnect(api_key="PG0BJ6Nl")
totp = pyotp.TOTP("2WZHOQAWJETSLLYNFXX23PFI7A")
print("pyotp",totp.now())
attempts = 5
while attempts > 0:
    attempts = attempts-1
    data = obj.generateSession("K256027","K@rthik88", totp.now())
    if data['status']:
        break
    tt.sleep(2)
