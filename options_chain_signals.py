...
for nse_cnt in range(0,len(nse_data)):
	...
	if nse_data.loc[nse_cnt,"Symbol"] == "BANKNIFTY":
		starting_strike_price = 40000
		ending_strike_price = 41000
		seq = 100
		symbol = "BankNifty"
		sql = "select distinct Datetime,Open,High,Low,Close,Volume from Stocks_data_1_minutes where cast(Datetime as date) = (select max(cast(Datetime as date)) from Stocks_data_1_minutes) and stock = 'BankNifty' and instrumenttype = 'FUTIDX'"
		# hist = {
		# 		"exchange":"NFO",
  #               "symboltoken":"51942",
  #               "interval":"ONE_MINUTE",
  #               "fromdate":str(todays_date)+ " 09:15",
  #               "todate":str(todays_date)+ " 15:30"
		# }


	else:
		starting_strike_price = 17000
		ending_strike_price = 18000
		seq = 50
		symbol = "Nifty"
		sql = "select distinct Datetime,Open,High,Low,Close,Volume from Stocks_data_1_minutes where cast(Datetime as date) = (select max(cast(Datetime as date)) from Stocks_data_1_minutes) and stock = 'Nifty' and instrumenttype = 'FUTIDX'"
		# hist = {
		# 		"exchange":"NFO",
  #               "symboltoken":"51951",
  #               "interval":"ONE_MINUTE",
  ...
  #               "todate":str(todays_date)+ " 15:30"
	...
	# print(hist_df)
	print(sql)
	hist_df = pd.read_sql(sql,con=engine)
	...
	for i in range(starting_strike_price,ending_strike_price,seq):
		# print(i)
		current_data = pd.read_csv('/Users/apple/Desktop/Python_Stocks_Automation/Options_data/'+str(symbol)+'/'+str(expiry_date)+"/"+str(i)+'.csv',index_col=False)
	...
	final_data = pd.DataFrame(pd.date_range("09:15", "15:30", freq="1min"))
	...
	call_data.to_csv('/Users/apple/Desktop/Python_Stocks_Automation/Options_data/'+str(symbol)+'/'+'call_data.csv',sep=',', header=True)
	...
	futures_df.to_csv('/Users/apple/Desktop/Python_Stocks_Automation/Options_data/'+str(symbol)+'/'+'futures_df.csv',sep=',', header=True)
	...
	temp_fut_data.to_csv('/Users/apple/Desktop/Python_Stocks_Automation/Options_data/'+str(symbol)+'/'+'Futures_testing.csv',sep=',', header=True)
	...
	Signals_df.to_csv('/Users/apple/Desktop/Python_Stocks_Automation/Options_data/'+str(symbol)+'/'+str(todays_date)+'_Options_Signals.csv',sep=',', header=True)
	futures_df.to_csv('/Users/apple/Desktop/Python_Stocks_Automation/Options_data/'+str(symbol)+'/'+str(todays_date)+'_Futures_Options_Signals.csv',sep=',', header=True)
